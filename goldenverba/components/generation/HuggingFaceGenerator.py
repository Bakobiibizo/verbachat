...
from goldenverba.components.generation.hf_models import (
    HuggingFace,
)

hf_options = HuggingFace()


class HuggingFace(Generator):
    """
    ...
    def __init__(self):
        super().__init__()
        self.name = "HuggingFace Generator"
        self.description = "Generator using a Huggingface model"
        self.requires_library = ["huggingface_hub", "transformers"]
        self.requires_env = ["HF_TOKEN", "HUGGINGFACE_TOKEN"]
        self.streamable = True
        self.model = None
        self.tokenizer = None
        self.device = None
        self.context_window = 3000
        self.list_options = hf_options.get_options()
        self.select_option = hf_options.select_option
        self.selected_model = hf_options.selected_option
        try:
            import torch
            ...
            def get_device():
                ...
                elif torch.backend_xla.is_available():
    ...
    async def generate_stream(
        ...
        try:
            ...
            mapped_input = {
                key: value.to(self.device) for key, value in user_input.items()
            }
            attention_mask = mapped_input.get(
                "attention_mask"
            )  # Get the attention mask, if provided
            input_len = mapped_input["input_ids"].shape[1]
            ...
            for output_len in range(output_length):
                ...
                output = await asyncio.to_thread(
                    lambda: self.model.generate(
                        input_ids=mapped_input["input_ids"],
                ...
                mapped_input["input_ids"] = torch.cat(
                    (mapped_input["input_ids"], current_token_id.view(1, 1)), dim=1
                )
